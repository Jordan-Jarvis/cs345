 ...
 *    Jordan Jarvis
 * Summary:
 *    Calculate matrix multiplication using threads
 ************************************************************************/
...
bool multiply(MatrixMultiplication & m, int numThreads)
{
   m.arrayDestination = new int[m.numRowLHS * m.numColRHS];
   //collapse combines the nested loops as if 
   //they were one and allows for paralell nested
   //loops.
   //using 'for' says for loop
   //share the struct
   ...
   for (int row = 0; row < m.numRowLHS; row++)
   {
      ...
      {
         m.arrayDestination[row * m.numColRHS + col] = 0; 
         // set the value to 0 for calculation
         for(int k = 0; k < m.numColLHS; k++)
         { 
            m.arrayDestination[row * m.numColRHS + col] 
               += m.arrayLHS[row * m.numColLHS + k] 
               * m.arrayRHS[k * m.numColRHS + col];
         }
      }
   } // after every thread is started
